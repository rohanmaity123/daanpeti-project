        ...
        return (
            <div className="flex gap-3 w-full">
                ...
                <button
                    ...
                    className="flex items-center justify-center gap-1.5 rounded-xl px-4 py-3.5 text-sm font-bold text-white transition-opacity hover:opacity-80"
    ...
    return (
        <div className="gradient-page pb-28 lg:pb-10">
                    ...
                    <div className="space-y-4">
                        ...
                        >
                            <div className="relative overflow-hidden rounded-[22px]">
                                ...
                                <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gradient-to-t from-black/55 to-transparent p-4">
                                    ...
                                    {isClaimed && <span className="glass-surface rounded-full px-3 py-1.5 text-xs font-bold text-white">Claimed</span>}
                                </div>
                            </div>
                        </button>
                        ...
                        <div className="glass-card rounded-[28px] p-5">
                            <div className="flex flex-wrap items-center gap-2">
                                <span className={`inline-block rounded-full px-2.5 py-1 text-xs font-semibold ${categoryColors[item.category]}`}>{categoryLabels[item.category]}</span>
                                {!isClaimed && <span className="feed-free-badge inline-block rounded-full px-2.5 py-1 text-xs font-bold text-white">🆓 Bilkul FREE</span>}
                            </div>
                    ...
                    <div className="space-y-4">
                            ...
                            <div className="mt-4 space-y-3">
                                ...
                                <div className="item-detail-donor-row">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <p className="mt-1 text-sm font-semibold text-white/80">{isDonor ? item.whatsapp_number : 'WhatsApp se direct connect karein'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="hidden rounded-[28px] p-5 lg:block glass-card">
                            ...
                            <p className="mt-2 text-sm text-white/65">
                                {isClaimed ? 'Yeh item already claimed ho chuka hai.' : isDonor ? 'Recipient milte hi item ko claimed mark kar dein.' : 'Interested ho? Donor ko WhatsApp par message bhejo.'}
                            </p>
            ...
            <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)} fullWidth maxWidth="xs"
                ...
                PaperProps={{ className: 'glass-dialog-paper rounded-[38px]', style: { background: 'linear-gradient(180deg, rgba(255,255,255,0.12), rgba(255,255,255,0.06))', backdropFilter: 'blur(22px) saturate(180%)', WebkitBackdropFilter: 'blur(22px) saturate(180%)', border: '1px solid rgba(255,255,255,0.16)', boxShadow: '0 24px 80px rgba(0,0,0,0.38)', color: 'rgba(255,255,255,0.96)', margin: '16px', width: 'calc(100% - 32px)', maxWidth: '420px' } }}
            >
                <DialogTitle sx={{ color: 'rgba(255,255,255,0.98)', fontWeight: 700 }} className="px-6 pt-6 text-base">Has this item been picked up?</DialogTitle>
                <DialogContent className="px-6 pb-5">
                    <DialogContentText sx={{ color: 'rgba(255,255,255,0.78) !important' }}>Kya yeh item kisi ne le liya? Once marked, it will show as claimed.</DialogContentText>
                </DialogContent>
                <DialogActions className="flex flex-col gap-3 px-6 pb-6 sm:flex-row sm:gap-2">
                    <button onClick={() => setConfirmOpen(false)} className="w-full rounded-2xl border border-white/12 bg-white/6 px-4 py-3 text-sm font-semibold text-white hover:bg-white/10 sm:flex-1">Cancel</button>
                    <button onClick={handleConfirmClaim} className="my-items-claim-button w-full rounded-2xl px-4 py-3 text-sm font-bold text-white hover:opacity-90 sm:flex-1">Yes, mark as claimed</button>
                </DialogActions>
            </Dialog>
            ...
            >
                <DialogContent className="p-2 sm:p-3">
                    <button onClick={() => setImageZoomOpen(false)} className="absolute top-3 right-3 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-black/40 hover:bg-black/60 backdrop-blur-md">
                        <X className="h-5 w-5 text-white" />
                    </button>
                    <img src={item?.image_url} alt={item?.name} className="max-h-[82vh] w-full rounded-[22px] object-contain" />
                </DialogContent>
            </Dialog>
        </div>
    );
