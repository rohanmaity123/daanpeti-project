...
/* ── Coupon card ── */
function CouponCard({
  coupon, userPts, userTier, onRedeem, myRedeemed,
}) {
  ...
  const tierInfo = TIERS[coupon.tier_required];
  ...
  return (
    ...
    >
      ...
      <div className="h-1 w-full" style={{
        background: alreadyHad
          ? 'linear-gradient(90deg,#639922,#1D9E75)'
          : canRedeem
      ...
      <div className="p-4 flex flex-col flex-1 gap-3">
        ...
        {alreadyHad && (
          <div className="rounded-xl p-3 space-y-2" style={{ background: 'rgba(19,136,8,0.06)', border: '1px solid rgba(19,136,8,0.15)' }}>
            <p className="text-xs font-semibold text-primary">✅ Redeemed! Copy your code:</p>
        ...
        {!alreadyHad && !canTier && (
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground rounded-xl bg-muted px-3 py-2">
            ...
            Reach {tierInfo.label} tier to unlock
          </div>
        )}
        {!alreadyHad && canTier && !canAfford && (
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground rounded-xl bg-muted px-3 py-2">
            <Zap className="h-3.5 w-3.5 shrink-0" />
            Need {coupon.points_required - userPts} more points
          </div>
        )}
        ...
        {!alreadyHad && canRedeem && (
          <motion.button
            onClick={handleRedeem} disabled={loading} whileTap={{ scale: 0.97 }}
            className="mt-auto w-full flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-bold text-white disabled:opacity-60 transition-all"
  ...
  return (
    <div className="mx-auto max-w-sm px-4 pt-10 pb-28 text-center">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <motion.button onClick={async () => { setLoading(true); try { await signInWithGoogle(); } catch { setLoading(false); } }}
...
export default function RewardsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setDataLoading(true);
      const [{ data: ptsData }, { data: couponData }, { data: redeemedData }, { data: txData }] =
        await Promise.all([
          supabase.from('user_points').select('*').eq('user_id', user.id).maybeSingle(),
          supabase.from('coupons').select('*').eq('is_active', true).order('points_required'),
          supabase.from('user_coupons').select('*, coupons(brand,brand_logo_emoji,title,discount_text)').eq('user_id', user.id),
          supabase.from('point_transactions').select('*').eq('user_id', user.id).order('created_at', { ascending: false }).limit(20),
        ]);
      setPts(ptsData);
      setCoupons(couponData ?? []);
      setMyRedeemed(redeemedData ?? []);
      setTxHistory(txData ?? []);
  ...
  const handleRedeem = async (couponId) => {
    ...
    if (error || !data?.success) {
      showToast(data?.error ?? error?.message ?? 'Redemption failed');
      return;
    }
    showToast(`✅ Redeemed! Code: ${data.code}`);
    // refresh data
    const [{ data: ptsData }, { data: redeemedData }, { data: txData }] = await Promise.all([
      supabase.from('user_points').select('*').eq('user_id', user?.id).maybeSingle(),
      supabase.from('user_coupons').select('*, coupons(brand,brand_logo_emoji,title,discount_text)').eq('user_id', user?.id),
      supabase.from('point_transactions').select('*').eq('user_id', user?.id).order('created_at', { ascending: false }).limit(20),
    ]);
  ...
  const lifetimePts = pts?.lifetime_points ?? 0;
  const nextTierPts = tierInfo.next;
  const progressPct = nextTierPts
    ? Math.min(100, Math.round(((lifetimePts - tierInfo.min) / (nextTierPts - tierInfo.min)) * 100))
    : 100;
      ...
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            className="fixed top-20 left-1/2 -translate-x-1/2 z-50 rounded-2xl px-5 py-3 text-sm font-bold text-white shadow-lg"
            style={{ background: toast.startsWith('✅') ? 'linear-gradient(135deg,#138808,#1D9E75)' : '#E24B4A' }}>
            {toast}
          </motion.div>
        )}
      </AnimatePresence>
          ...
          <div>
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-extrabold text-foreground">
                <Counter target={totalPts} />
              </span>
              <span className="text-sm text-muted-foreground">points available</span>
            </div>
            <p className="text-xs text-muted-foreground mt-1">{lifetimePts} lifetime points</p>
          </div>
          {/* Right: stats */}
          <div className="flex gap-4">
            {[
              { icon: Gift, label: 'Donated', val: pts?.items_donated ?? 0 },
              { icon: Award, label: 'Coupons', val: pts?.coupons_redeemed ?? 0 },
            ].map(s => (
              <div key={s.label} className="text-center">
                <p className="text-xl font-extrabold text-foreground"><Counter target={s.val} /></p>
        ...
        {nextTierPts && (
          <div className="mt-4">
            <div className="flex justify-between text-xs text-muted-foreground mb-1.5">
              <span>Progress to {TIERS[Object.keys(TIERS).find(k => TIERS[k].min === nextTierPts)]?.label ?? 'next tier'}</span>
              <span>{lifetimePts} / {nextTierPts} pts</span>
            </div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {nextTierPts - lifetimePts} more points to level up
            </p>
          </div>
        )}
        {!nextTierPts && (
          <p className="mt-3 text-xs font-semibold" style={{ color: tierInfo.color }}>
            👑 You are at the highest tier! Maximum rewards unlocked.
          </p>
        )}
      </motion.div>
        ...
        {Object.entries(TIERS).map(([key, t]) => {
          ...
          return (
            <div key={key} className="rounded-2xl p-3 text-center"
              style={{ background: t.bg, border: isActive ? `2px solid ${t.color}` : '1px solid transparent', opacity: unlocked ? 1 : 0.5 }}>
              ...
              {isActive && <p className="text-[9px] font-bold mt-1" style={{ color: t.color }}>YOU ARE HERE</p>}
            </div>
          );
        })}
      </motion.div>
      ...
      {activeTab === 'redeem' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 mb-4" style={{ scrollbarWidth: 'none' }}>
            {[['all', 'All', '#138808'], ['seedling', '🌱 Seedling', '#639922'], ['helper', '💧 Helper', '#185FA5'], ['star', '⭐ Star', '#854F0B'], ['legend', '👑 Legend', '#534AB7']].map(([val, label, color]) => (
          ...
          {dataLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm font-bold text-foreground">No coupons available</p>
            </div>
          ) : (
      ...
      {activeTab === 'earn' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
          {[
            ...
            { icon: '✅', title: 'Item gets claimed', pts: '+5 pts', desc: 'When someone picks up your donated item.' },
            ...
            { icon: '🎂', title: 'Birthday bonus', pts: '+50 pts', desc: 'Celebrate your birthday with extra points! (Coming soon)' },
          ].map((item, i) => (
          ...
          {/* Tier perks */}
          <div className="rounded-2xl bg-card card-shadow p-5 mt-2">
