...
import { z } from 'zod';
import { Camera, Loader2, CheckCircle2, AlertCircle, X, LogIn, Lock, ArrowLeft } from 'lucide-react';
...
/* ── Main page — handles both POST and EDIT ── */
export default function PostItemPage() {
    const navigate = useNavigate();
    const { itemId } = useParams();           // present only on /edit/:itemId
    const isEditMode = Boolean(itemId);
    const { user, loading: authLoading } = useAuth();

    const [imageFile, setImageFile] = useState(null);
    const [imagePreview, setImagePreview] = useState(null);
    const [existingImage, setExistingImage] = useState(null); // original URL when editing
    ...
    /* ── In edit mode, load existing item and pre-fill the form ── */
    useEffect(() => {
        ...
        load();
    }, [isEditMode, itemId, user]);

    /* ── Auth / data loading gates ── */
    if (authLoading || fetchingItem) return (
    ...
    const handleImageChange = (e) => {
        ...
        setExistingImage(null); // replaced
    };
    ...
    const onSubmit = async (values) => {
        ...
        try {
            ...
            let image_url = existingImage ?? null; // keep existing if no new file
            ...
            if (isEditMode) {
                ...
                setAfterPostData({ ...payload, status: 'available', user_id: user.id });
                setSubmitState('success');
                reset();
                removeImage();
    ...
    return (
        <>
            <Helmet>
                <title>{isEditMode ? 'Edit Item — DaanGuru' : 'Post Item for Donation — DaanGuru'}</title>
                <meta name="description" content="Donate items you don't need anymore. Post books, clothes, furniture, electronics and more for free." />
                <link rel="canonical" href={`https://www.daanguru.in/${isEditMode ? `edit/${itemId}` : 'post-item'}`} />
            </Helmet>

            <div className="mx-auto max-w-3xl px-4 pt-5 pb-28 lg:pb-10">

                {/* Share modal — only on new post */}
                {!isEditMode && (
                    <ShareDonationModal
                        open={confirmOpen}
                ...
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                    {isEditMode && (
                        <Link to="/my-items" className="mb-3 inline-flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors">
                            <ArrowLeft className="h-4 w-4" />Back to My Items
                        </Link>
                    )}
                    <h1 className="text-2xl lg:text-3xl font-extrabold text-foreground">
                        {isEditMode ? '✏️ Item Edit Karo' : 'Daan Karo 🎁'}
                    </h1>
                    <p className="text-sm text-muted-foreground mt-0.5">
                        {isEditMode
                            ? 'Apne item ki details update karo.'
                    ...
                    {isEditMode ? (
                        <div className="mt-2 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold"
                            ...
                            ✏️ Edit mode — changes will update your listing
                        </div>
                    ) : (
                        <div className="mt-2 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold"
                ...
                <form className="mt-5" onSubmit={handleSubmit(onSubmit)} noValidate>
                    <div className="lg:grid lg:grid-cols-2 lg:gap-8 space-y-5 lg:space-y-0">
                                ...
                                {imagePreview ? (
                                    <>
                                        ...
                                        <div className="absolute inset-0 bg-black/0 hover:bg-black/30 transition-colors flex items-center justify-center">
                                            <span className="opacity-0 hover:opacity-100 transition-opacity text-xs font-bold text-white bg-black/50 px-3 py-1.5 rounded-full">
                                                Tap to change
                                            </span>
                                        </div>
                                        <button type="button" onClick={(e) => { e.stopPropagation(); removeImage(); }}
                                            className="absolute top-2 right-2 rounded-full bg-black/50 p-1.5 text-white hover:bg-black/70 z-10">
                                            <X className="h-4 w-4" />
                                        </button>
                                    </>
                                ) : (
                        ...
                        <div className="space-y-4">
                            ...
                            <motion.div custom={4} variants={fieldVariants} initial="hidden" animate="show">
                                ...
                                <textarea rows={3} placeholder="Item ke baare mein thoda batao..."
                                    ...
                                    <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
                                        ...
                                        {[
                                            ...
                                            <span key={tag} className="text-[11px] font-semibold px-2.5 py-1 rounded-full"
                                                style={{ background: 'rgba(19,136,8,0.08)', color: '#138808' }}>{tag}</span>
                                        ))}
                                    </motion.div>
                                )}
                            </motion.div>
                            ...
                            <motion.div custom={6} variants={fieldVariants} initial="hidden" animate="show">
                                <label className="text-sm font-bold text-foreground">
                                    Pincode *
                                    {resolvedPlace?.pincode && <span className="ml-2 text-[10px] font-normal text-primary">Auto-filled ✓</span>}
                                </label>
                    ...
                    <div className="mt-6 flex flex-col sm:flex-row gap-3">
                        <motion.button type="submit"
                            ...
                            className="w-full sm:w-auto sm:px-14 rounded-xl py-3.5 text-sm font-bold text-white transition-all disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            style={{ background: isEditMode ? 'linear-gradient(135deg,#ef9f27,#d85a30)' : 'linear-gradient(135deg,#138808,#1D9E75)' }}>
                            {submitState === 'loading' && <><Loader2 className="h-4 w-4 animate-spin" />{isEditMode ? 'Saving...' : 'Posting...'}</>}
                            {submitState === 'success' && <><CheckCircle2 className="h-4 w-4" />{isEditMode ? 'Saved! Redirecting...' : 'Posted! Redirecting...'}</>}
                            {(submitState === 'idle' || submitState === 'error') && (
                                isEditMode ? '💾 Save Changes' : '🎁 Post Item — Daan Karo!'
                            )}
                        </motion.button>

                        {isEditMode && (
                            <Link to="/my-items"
                                className="w-full sm:w-auto rounded-xl py-3.5 px-6 text-sm font-semibold text-foreground flex items-center justify-center transition-colors hover:bg-muted"
