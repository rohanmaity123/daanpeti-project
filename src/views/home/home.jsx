                    ...
                    <div className="space-y-1">
                        {filterCategories.map((cat) => (
                            <button
                                ...
                                className={`w-full text-left rounded-xl px-3 py-2 text-sm font-semibold transition-colors ${activeCategory === cat.value
            ...
            <div className="mt-4 flex gap-2 overflow-x-auto px-4 pb-1 lg:hidden" style={{ scrollbarWidth: 'none' }}>
                {filterCategories.map((cat) => (
                    <button
                        key={cat.value}
                        onClick={() => setActiveCategory(cat.value)}
                        className={`shrink-0 rounded-full px-3.5 py-1.5 text-xs font-semibold transition-colors ${activeCategory === cat.value
                            ? 'bg-primary text-primary-foreground'
                            ...
                            {pageCount > 1 && (
                                <div className="mx-4 lg:mx-0 mt-5 flex flex-wrap items-center justify-center gap-2">
                                    <button
                                        ...
                                        className="rounded-full border border-border bg-card px-4 py-2 text-sm font-semibold text-foreground transition hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50"
                                    >
                                        Previous
                                    </button>
                                    {Array.from({ length: pageCount }, (_, index) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`rounded-full px-4 py-2 text-sm font-semibold transition ${currentPage === page ? 'bg-primary text-primary-foreground' : 'bg-card text-foreground hover:bg-secondary'}`}
                                            >
                                                {page}
                                            </button>
                                        );
